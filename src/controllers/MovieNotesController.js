const knex = require('../database/knex')
const ErrorAlert = require('../utils/ErrorAlert')

class MovieNotesController {
  //title = varchar | desc = varchar | rating = int | user_id = int
  async create(request, response) {
    const user_id = request.user.id
    const { title, description, rating, tags } = request.body

    const userExists = await knex('users').where({ id: user_id }).first()

    if (!userExists) {
      throw new ErrorAlert('Inexistent user!')
    }

    const note_id = await knex('movie_notes').insert({
      title,
      description,
      rating,
      user_id
    })

    console.log(note_id)

    const tagsInsert = tags.map(tag => {
      return {
        note_id,
        user_id,
        name: tag
      }
    })

    await knex('movie_tags').insert(tagsInsert)

    return response.json()
  }

  async index(request, response) {
    //innerjoin... já aproveita e vê essa parte da aula tbm! =X
    const { title, tags } = request.query
    const user_id = request.user.id

    let notes

    if (tags) {
      const filterTags = tags.split(',').map(tag => tag.trim())

      notes = await knex('movie_tags')
        .select('movie_notes.id', 'movie_notes.title', 'movie_notes.user_id')
        .where('movie_notes.user_id', user_id)
        .whereLike('movie_notes.title', `%${title}%`)
        .whereIn('name', filterTags)
        .innerJoin('movie_notes', 'movie_notes.id', 'movie_tags.note_id')
        .orderBy('movie_notes.title')
    } else {
      notes = await knex('movie_notes')
        .where({ user_id })
        .whereLike('title', `%${title}%`)
        .orderBy('title')
    }

    const userTags = await knex('movie_tags').where({ user_id })
    const notesAndTags = notes.map(note => {
      const noteTags = userTags.filter(tag => tag.note_id === note.id)

      return {
        ...note,
        tags: noteTags
      }
    })

    return response.json(notesAndTags)
  }

  async show(request, response) {
    const { id } = request.params

    const note = await knex('movie_notes').where({ id }).first()
    const tags = await knex('movie_tags').where({ note_id: id }).orderBy('name')

    return response.json({
      ...note,
      tags
    })
  }

  async delete(request, response) {
    const { id } = request.params

    const noteExist = await knex('movie_notes').where({ id }).first()

    if (!noteExist) {
      throw new ErrorAlert('This note does not exist!')
    }

    await knex('movie_notes').where({ id }).del()

    return response.json({ message: 'This note was deleted!' })
  }
}

module.exports = MovieNotesController
